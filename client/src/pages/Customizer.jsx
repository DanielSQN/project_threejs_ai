...
const I = {
  ...
  center: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 3 21 3 21 9" /><polyline points="9 21 3 21 3 15" /><line x1="21" y1="3" x2="14" y2="10" /><line x1="3" y1="21" x2="10" y2="14" /></svg>,
...
const Customizer = () => {
  ...
  return (
    <div className="customizer">
      ...
      {/* top bar: back (left) + recenter (right) */}
      <header className="cz-top">
        ...
        <button className="cz-center" onClick={recenter} aria-label="Centrar" title="Centrar">{I.center} <span className="cz-center-t">Centrar</span></button>
      </header>

      {/* rotate hint, centered over the shirt, fades out */}
      {showHint && <div className="cz-rotate-hint">{I.hand} Arrastra para rotar</div>}

      {/* bottom area: front/back toggle + docked menu (tools + add to cart) */}
      <div className="cz-bottom">
        ...
        <div className={`cz-menu ${expanded ? 'expanded' : ''}`}>
          ...
          <button className="cz-addcart" onClick={handleAddToCart}>
            {I.bagPlus} Agregar al carrito · Talla {snap.size}
          </button>
        </div>
      </div>
    </div>
